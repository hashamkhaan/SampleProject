...
import AddStreets from '../Booking/AddStreets';
...
const TablePage = () => {
	...
	useEffect(() => {
		...
		Axios.get(`${baseURL}/getStreets?pageNo=1&&records=10&&sort=asc&&colName=id`)
			.then((response) => {
				console.log('GET getStreets::::::::', response.data);
				setPlots(response.data.streets);
	...
	// const { items, requestSort, getClassNamesFor } = useSortableData(Plots);
	const { items, requestSort, getClassNamesFor } = useSortableData(Plots);
						...
						<Card>
							<CardHeader>
								<CardLabel icon='Assignment'>
									<CardTitle>Street List</CardTitle>
								</CardLabel>
							</CardHeader>
