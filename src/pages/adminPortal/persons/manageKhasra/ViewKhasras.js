...
const SubGroups = (props) => {
	...
	const toggleStatus = (id) => {
		Axios.get(`${baseURL}/makeKhasraActiveOrInactive?employee_id=${id}`)
			.then((response) => {
	...
	const deleteKhasra = (id) => {
		Axios.get(`${baseURL}/deleteKhasra?person_id=${id}`)
			.then((response) => {
	...
	const getEditingKhasra = (id) => {
		...
		Axios.get(`${baseURL}/editKhasra?khasra_id=${id}`)
			.then((response) => {
				setEditingKhasraData(response.data.khasra);
						...
						<tbody>
							{onCurrentPageData.map((item) => (
								<tr key={item.id}>
									...
									<td>
										{item.size.kanal}-{item.size.marla}-{item.size.sarsai}-
										{item.size.feet}
									</td>
				...
				<ModalBody>
					<div className='row g-4'>
						<div className='col-12'>
							<Card>
								<CardBody>
									Are you sure you want to delete Khasra? <br />
									ID: {deletingKhasraId}
									<br />
					...
					<ModalTitle id='editVoucher'>
						...
						<CardHeader>
							<CardLabel icon='Edit' iconColor='info'>
								<CardTitle>
									Editing Khasra no: {editingKhasraData?.khasra_no} <br />
									<small> Khasra ID: {editingKhasraData?.khasra_id}</small>
								</CardTitle>
							</CardLabel>
						</CardHeader>
					</ModalTitle>
