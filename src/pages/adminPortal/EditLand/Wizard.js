...
const Wizard = ({ children, onSubmit, isHeader, color, stretch, ...props }) => {
	...
	const submitForm = (dataSubmit) => {
		...
		Axios.post(`${baseURL}/updateLand?land_id=${store.data.land_id}`, formData, {
			headers: {
			...
			.then((res) => {
				...
				if (res.data.status === 'ok') {
					...
					navigate(`${subDirForNavigation}adminPortal/lands/viewPurchaseLand`, {
						replace: true,
					});
			...
			<Button
				...
				onClick={() => {
					try {
						...
						showNotification('Error ', 'Error updating record', 'danger');
						// eslint-disable-next-line no-console
