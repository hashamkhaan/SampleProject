...
const AddPlots = (props) => {
	...
	useEffect(() => {
		...
		Axios.get(`${baseURL}/getStreetsDropdown`)
			...
			// eslint-disable-next-line no-console
			.catch((err) => console.log(err));
		getPlotSize(formik.values.block_id);
		...
		<CardBody>
			<div className='row'>
				<div className='col-md-4'>
					<FormGroup id='block_id' label='Blocks'>
						<Select
							className='col-md-12'
							options={GetBlocksArray}
							// isLoading={crAccountLoading}
							value={
								formik.values.block_id !== null &&
								GetBlocksArray.find((c) => c.id === formik.values.block_id)
							}
							onChange={(e) => {
								formik.setFieldValue('block_id', e.value);
								getPlotSize(e.value);
							}}
							onBlur={formik.handleBlur}
							isValid={formik.isValid}
							isClearable
							isTouched={formik.touched.block_id}
							invalidFeedback={formik.errors.block_id}
							validFeedback='Looks good!'
						/>
					</FormGroup>
					{formik.errors.block_id && (
						<p
							style={{
								color: 'red',
							}}>
				...
				<div
					...
					<div className='col-md-9 border-end'>
						<div className='row' key={formik.values.file_no}>
							...
							<div className='col-md-2'>
								<FormGroup id='plot_no' label='Plot No' className='col-md-12'>
									<Input
										type='number'
										min='1'
										onWheel={(e) => e.target.blur()}
										...
										isValid={formik.isValid}
										isTouched
										invalidFeedback={formik.errors.plot_no}
										validFeedback='Looks good!'
									/>
								</FormGroup>
							</div>
							<div className='col-md-2'>
								<FormGroup id='extension' label='Extension' className='col-md-12'>
									<Input
										onChange={formik.handleChange}
										onBlur={formik.handleBlur}
