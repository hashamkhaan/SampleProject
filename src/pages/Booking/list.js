...
const TablePage = () => {
	...
	const [GetBoooking, setGetBoooking] = useState([]);
	const [GetBoookingLoading, setGetBoookingLoading] = useState(true);
	const [printreport, setPrintreport] = useState([]);

	// const { items, requestSort, getClassNamesFor } = useSortableData(GetBoooking);
	const { items, requestSort, getClassNamesFor } = useSortableData(GetBoooking);
	const onCurrentPageData = dataPagination(items, currentPage, perPage);
	const { selectTable, SelectAllCheck } = useSelectTable(onCurrentPageData);
	useEffect(() => {
		setGetBoookingLoading(true);
		Axios.get(`${baseURL}/getBooking?pageNo=1&&records=999999&&sort=asc&&colName=id`)
			.then((response) => {
				console.log('GET BOOKING::::::::', response.data);
				setGetBoooking(response.data.Bookings);
				setGetBoookingLoading(false);
			})
